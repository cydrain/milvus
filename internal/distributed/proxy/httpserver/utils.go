...
func convertInt8VectorToIntArray(vector [][]int8, dim int64) ([]int32, error) {
	...
	for _, arr := range vector {
		...
		for i := int64(0); i < dim; i++ {
			intArray = append(intArray, int32(arr[i]))
		}
	}
	return intArray, nil
}
...
func anyToColumns(rows []map[string]interface{}, validDataMap map[string][]bool, sch *schemapb.CollectionSchema, inInsert bool) ([]*schemapb.FieldData, error) {
	...
	for name, column := range nameColumns {
		...
		case schemapb.DataType_Int8Vector:
			dim := nameDims[name]
			arr, err := convertInt8VectorToIntArray(column.([][]int8), dim)
			if err != nil {
				return nil, err
			}
			colData.Field = &schemapb.FieldData_Vectors{
...
func serializeInt8Vectors(vectorStr string, dataType schemapb.DataType, dimension, bytesLen int64, int8ArrayToBytesFunc func([]int8) []byte) ([][]byte, error) {
	var int8Values [][]int8
...
func convertQueries2Placeholder(body string, dataType schemapb.DataType, dimension int64) (*commonpb.PlaceholderValue, error) {
	...
	case schemapb.DataType_Int8Vector:
		...
		values, err = serializeInt8Vectors(gjson.Get(body, HTTPRequestData).Raw, dataType, dimension, dimension, typeutil.Int8ArrayToBytes)
	case schemapb.DataType_VarChar:
...
func buildQueryResp(rowsNum int64, needFields []string, fieldDataList []*schemapb.FieldData, ids *schemapb.IDs, scores []float32, enableInt64 bool) ([]map[string]interface{}, error) {
	...
	if rowsNum == int64(0) { // always
		if columnNum > 0 {
			...
			case schemapb.DataType_Int8Vector:
				rowsNum = int64(len(fieldDataList[0].GetVectors().GetInt8Vector().Data)) / fieldDataList[0].GetVectors().GetDim()
			default:
	...
	for i := int64(0); i < rowsNum; i++ {
		...
		if columnNum > 0 {
			for j := 0; j < columnNum; j++ {
				...
				case schemapb.DataType_Int8Vector:
					row[fieldDataList[j].FieldName] = fieldDataList[j].GetVectors().GetInt8Vector().Data[i*fieldDataList[j].GetVectors().GetDim() : (i+1)*fieldDataList[j].GetVectors().GetDim()]
				case schemapb.DataType_Array:
					if len(fieldDataList[j].ValidData) != 0 && !fieldDataList[j].ValidData[i] {
						row[fieldDataList[j].FieldName] = nil
						continue
