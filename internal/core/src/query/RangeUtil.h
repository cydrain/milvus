...
ReGenRangeSearchResult(const knowhere::DatasetPtr res,
    ...
    for (auto i = 0; i < nq; i++) {
        if (metric_type == knowhere::metric::IP) {
            cnt += GetRangeSearchResultForOneNq<true>(topk, low_bound, high_bound, lims[i + 1] - lims[i],
                                                      distances + lims[i], labels + lims[i], p_dist + i * topk,
                                                      p_id + i * topk, bitset);
        } else {
            cnt += GetRangeSearchResultForOneNq<false>(topk, low_bound, high_bound, lims[i + 1] - lims[i],
                                                       distances + lims[i], labels + lims[i], p_dist + i * topk,
                                                       p_id + i * topk, bitset);
        }
    }
